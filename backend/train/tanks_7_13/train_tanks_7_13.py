...
def train_tank_7_13(data_path, model_dir):
    ...
    Uses corrected level logic: actual_level = MaxFt - raw_sensor.
    """
    ...
    try:
        ...
        max_ft = TANK_MAX_LEVELS.get(actual_tank_id.lower().replace(' ', '_'), 25.0)

        # ─── CORRECTED LEVEL LOGIC ───
        # actual_level = MaxFt - raw_sensor_value
        ...
        logger.info(f"Tank 7-13 Training complete for {data_path}. config.pkl saved.")
        ...
        logger.error(f"Tank 7-13 Training failed: {e}")
        return None
